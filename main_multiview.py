...
def dev_epoch_multiview(dev_loader, model, device, weighted_views: Dict[str, float] = {'1': 1.0, '2': 1.0, '3': 1.0, '4': 1.0}, ce_weight=[0.1, 0.9]):
        ...
        for batch in pbar:
            ...
            for view, (batch_x, batch_y) in batch.items():
                ...
                batch_loss = criterion(
                    batch_out, batch_y) * weighted_views[view]
                ...
                # Store batch metrics
                loss_detail[f'dev_loss/view_{view}'] = batch_loss.item() / \
                    batch_size
    ...
    for view in metrics:
        ...
        view_accuracy = 100. * \
            metrics[view]['correct'] / metrics[view]['total']
            ...
            while not_improving < args.num_epochs:
                ...
                if not args.is_multiview:
                    ...
                    train_epoch_multiview(
                        train_loader, model, optimizer, device, weighted_views)
                    val_loss, _ = dev_epoch_multiview(
                        dev_loader, model, device, weighted_views)
