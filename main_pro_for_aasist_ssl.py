...
from utils import read_metadata_pro, read_metadata
import numpy as np
from tqdm import tqdm
from data_utils_multiview import Dataset_var_eval
...
if __name__ == '__main__':
    ...
    model_tag = 'AASIST_SSL_{}_{}_{}_ES{}_H{}_NE{}_KS{}_AUG{}_w_sin_pos'.format(
    ...
    label_trn, files_id_train = read_metadata_pro( dir_meta =  os.path.join(args.database_path,'ASVspoof2019_LA_train_augmented/protocol.txt'))
    print('no. of training trials',len(files_id_train))
    
    train_set=Dataset_train(args,list_IDs = files_id_train,labels = label_trn,base_dir = os.path.join(args.database_path, 'ASVspoof2019_LA_train_augmented/'),algo=args.algo, format='.wav')
    ...
    labels_dev, files_id_dev = read_metadata_pro( dir_meta =  os.path.join(args.database_path,'ASVspoof2019_LA_dev_augmented/protocol.txt'))
    print('no. of validation trials',len(files_id_dev))

    dev_set = Dataset_train(args,list_IDs = files_id_dev,
		    labels = labels_dev,
		    base_dir = os.path.join(args.database_path,'ASVspoof2019_LA_dev_augmented/'), algo=args.algo, format='.wav')
    ...
    for tracks in eval_tracks:
        if not os.path.exists('Scores/{}/{}.txt'.format(tracks, model_tag)):
            ...
            if args.var:
                ...
                eval_set=Dataset_var_eval(list_IDs = file_eval,base_dir = os.path.join(args.database_path+'ASVspoof2021_{}_eval/'.format(tracks,tracks)),track=tracks)
                produce_evaluation_file(eval_set, model, device, 'Scores/{}/{}.txt'.format(tracks, model_tag), batch_size=1)
            else:
                print("Cut: ", args.cut)
                eval_set=Dataset_eval(list_IDs = file_eval,base_dir = os.path.join(args.database_path+'ASVspoof2021_{}_eval/'.format(tracks,tracks)),track=tracks, cut=args.cut)
                produce_evaluation_file(eval_set, model, device, 'Scores/{}/{}.txt'.format(tracks, model_tag), batch_size=args.batch_size)
        else:
