...
from data_utils import Dataset_train_cnsl as Dataset_train, Dataset_eval
...
if __name__ == '__main__':
    ...
    # Eval
    parser.add_argument('--n_mejores_loss', type=int,
                        default=5, help='save the n-best models')
    ...
    if args.dataset == 'asvspoof':
        ...
        dev_set = Dataset_train(args, list_IDs=files_id_dev,
                                labels=labels_dev,
                                base_dir=os.path.join(args.database_path+'{}_{}_dev/'.format(prefix_2019.split('.')[0], args.track)), algo=args.algo, cut=args.cut, random_start=args.random_start)
        ...
        files_id_train, label_trn = read_metadata_other(
            dir_meta=args.protocols_path, is_train=True)
        print('no. of training trials', len(files_id_train))

        train_set = Dataset_train(args, list_IDs=files_id_train, labels=label_trn, base_dir=os.path.join(
            args.database_path), algo=args.algo, format='', cut=args.cut, random_start=args.random_start)
        train_loader = DataLoader(
            train_set, batch_size=args.batch_size, num_workers=10, shuffle=True, drop_last=True)
        ...
        files_id_dev, labels_dev = read_metadata_other(
            dir_meta=args.protocols_path, is_dev=True)
        print('no. of validation trials', len(files_id_dev))

        dev_set = Dataset_train(args, list_IDs=files_id_dev,
                                labels=labels_dev,
                                base_dir=os.path.join(args.database_path), algo=args.algo, format='', cut=args.cut, random_start=args.random_start)
        ...
        if args.train:
            ...
            while not_improving < args.num_epochs:
                print('######## Epoca {} ########'.format(epoch))
                ce_weight = [0.5, 0.5]
                train_epoch(train_loader, model, args.lr,
                            optimizer, device, weight=ce_weight)
                val_loss = evaluate_accuracy(
                    dev_loader, model, device, weight=ce_weight)
                if val_loss < best_loss:
                    best_loss = val_loss
                    torch.save(model.state_dict(), os.path.join(
                        model_save_path, 'best.pth'))
