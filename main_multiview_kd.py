    ...
    for batch in pbar:
        ...
        for view_idx, (inputs, labels) in batch.items():
            ...
            if view_idx == 4:
                ...
                for k, v in logits_view_dict.items():    
                    loss_kd = loss_fn_kd(v, teacher_outputs, alpha, T)
                    batch_total_loss += loss_kd
                    running_kd_losses[f'view_{k}'] += loss_kd.item() * batch_size
...
if __name__ == '__main__':
    ...
    # Load teacher model
    teacher_model = Model(args, device)
    assert args.teacher_ckpt is not None, 'Teacher model checkpoint path not provided'
    teacher_model.load_state_dict(torch.load(args.teacher_ckpt))
    teacher_model = teacher_model.to(device)
    print('Teacher model loaded from {}'.format(args.teacher_ckpt))
        ...
        if args.train:
            ...
            while not_improving < args.num_epochs:
                ...
                if not args.is_multiview:
                    ...
                    weighted_views = {'1': 0.9, '2': 0.9, '3': 0.9, '4': 0.9}
                    train_epoch_multiview(
                        train_loader, model, teacher_model, optimizer, device, weighted_views)
                    val_loss, _ = dev_epoch_multiview(
                        dev_loader, model,teacher_model, device, weighted_views)
        ...
        if args.train:
            ...
            while not_improving < args.num_epochs:
                ...
                if not args.is_multiview:
                    ...
                    ce_weight = [0.3, 0.7]
                    train_epoch_multiview(
                        train_loader, model, optimizer, device, weighted_views, ce_weight)
                    val_loss, _ = dev_epoch_multiview(
                        dev_loader, model, device, weighted_views, ce_weight)
